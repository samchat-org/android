...
public class SendImageHelper {
	...
	public static void showImageAfterSelfImagePicker(Context context, Intent data, final Callback callback){		
		boolean isOrig = data.getBooleanExtra(Extras.EXTRA_IS_ORIGINAL, false);
		...
		for (PhotoInfo photoInfo : photos) {
			new ShowImageTask(context, isOrig, photoInfo, new Callback() {
	...
	public static class ShowImageTask extends AsyncTask<Void, Void, File> {
		...
		public ShowImageTask(Context context, boolean isOrig, PhotoInfo info,
				Callback callback) {
			this.context = context;
			this.isOrig = isOrig;
			this.info = info;
		...
		protected File doInBackground(Void... params) {
			...
			if (isOrig) {
				...
				// 生成缩略图
				File imageFile = new File(origMD5Path);
				ImageUtil.makeThumbnail2(context, imageFile);
